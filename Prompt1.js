...
function declareTributes (listOfTributes) {
    function writeTrubuteAnnouncement (singleTribute) {
        let anncouncment = `${singleTribute.name} from district; ${singleTribute.district}`
        return anncouncment;
    }
    let tributeAnnouncements = listOfTributes.map(writeTributeAnnouncment)
...
function doesThisToyContainChemicals (singleToy) {
    if (!singleToy.containsChemicals) {
        return singleToy
    }
}

let undamagedToys = toyInventory.filter(doesThisToyContainChemicles)
